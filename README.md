...
> 注：本机已**拆除夹爪、换成与笔固定的旋转连接件**，所以 `h` 现在是「笔旋转关节」；
> 但实际可动范围仍按 ±45，IK、轨迹校验和下位机软限位都按这个范围约束。画竖直笔的 IK
> 解需手腕 `w` 约 -93°，超 ±90 的部分被 clamp 到 -90（笔约 3° 恒定倾斜，画线无碍）；
> 若确认手腕舵机物理能转过 ±90，可放宽 `w`。
...
肘关节空程补偿。⚠️ 真发前确认下位机已按 4.3 启动、纸张就位、笔尖朝下；首次低速、人盯着。

...
_PI = np.pi
# 真机是 URDF 原始建模(右手系)关于 XZ 平面(Y->-Y)的整体镜像——原 CAD/URDF 很可能
# 由镜像装配导出。这里把链整体做 Y 反射，使模型忠实描述真机:b/w 旋向、零位笔尖朝向
# 全部与真机一致。反射配方: 平移 y->-y、固定 rpy(r,p,y)->rpy(-r,p,-y)、关节 sign 翻号、
# 笔偏移向量 y->-y。已数值验证 fk_new(q) ≡ diag(1,-1,1)·fk_old(q)。
# 每个关节: trans(origin) @ rpy(origin) @ Rz(sign*theta)
_JOINTS = [
    (_T(0, 0, 0.0493),                       np.eye(4),                 +1),  # j1
    (_T(0, 0, 0.0335),                       _rpy(-_PI/2, 0, 0),        +1),  # j2
    (_T(0.03564, -0.23002, 0),               np.eye(4),                 +1),  # j3
    (_T(0.24164, 0, 0),                      _rpy(0, -_PI/2, -_PI),     -1),  # j4
    (_T(0.0121, 0.02478, 0.01835),           _rpy(0, -_PI/2, -_PI/2),   -1),  # j5
]
_PEN_T = np.array([0.03431, 0.08012, -0.02672])    # URDF pen_tip 偏移 (y 已反射)
_NIB = np.array([0.0508886, -0.07813403, 0.030])  # 笔尖: 绕nib/tail中心累计+6°(右手绕link5 z轴)
_TAIL = np.array([0.0203114, 0.03793403, 0.030])  # 笔尾: 同步旋转

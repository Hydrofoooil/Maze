...
Maze/
  ...
  arm_sim/               # Isaac Sim 仿真与运动学验证
    arm_kinematics.py    # 5-DOF 正运动学、逆运动学、雅可比
  ...
  arm_real_server/       # 下位机：TCP 服务与串口控制
    ...
    force_bias_model.py  # 力矩零偏模型，服务笔压估计
...
\begin{lstlisting}[language=Python, caption={笔尖位置雅可比的数值计算}]
...
def ik(target, q0, command_offsets_deg=None):
...
当前轨迹文件 \texttt{maze\_planner/outputs/trajectory/trajectory.json} 中保存了 120 个轨迹点，其元信息如表 \ref{tab:traj_meta} 所示。
...
\begin{table}[H]
  ...
  \begin{tabular}{ll}
    ...
    轨迹点数 & 120 \\
    纸面中心距底座 & \texttt{paper\_cx = 0.33 m} \\
    点间时间 & \texttt{dt = 0.1 s} \\
    最大 IK 位置残差 & 约 $5.81 \times 10^{-10}$ mm \\
    最大笔轴偏离竖直角 & 约 $16.50^\circ$ \\
    \bottomrule
  \end{tabular}
\end{table}
...
\begin{figure}[H]
  ...
  \begin{tikzpicture}[
    ...
    \node[box, below=of c] (d) {pyserial 串口：COM10 @ 115200};
...
服务端接收到轨迹后会启动独立线程执行。当前实现采用 ``发送一个点，等待到位，再发送下一个点'' 的策略。到位判断不依赖单一的目标角误差，而是通过连续多次读取关节反馈，判断角度是否已经稳定，从而适应真实硬件状态反馈中存在的符号差异、噪声和 \texttt{move} 字段不稳定问题。
...
默认模式只进行规划、IK 求解、限位检查和轨迹文件保存，不连接机械臂。轨迹文件输出到 \texttt{maze\_planner/outputs/trajectory/trajectory.json}。
...
轨迹文件中保存的 IK 位置残差极小，说明在当前纸面位置设置下，机械臂运动学模型可以覆盖整条迷宫路径。最大笔轴偏离竖直角约为 $16.50^\circ$，说明部分位置受关节限位影响，笔无法完全竖直，但仍能保持可绘制姿态。
...
第三，笔压控制仍可进一步优化。服务端已经预留基于力矩反馈估计笔尖竖直力的 \texttt{force-z-adaptive} 逻辑，后续可通过标定提升笔压一致性。

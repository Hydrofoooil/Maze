...
3. **arm_real**：通过 TCP 把规划好的关节轨迹下发给**真实机械臂**执行。上位机（本机）跑 `arm_real_client`（`draw_maze_real.py` 串起「迷宫规划→IK→关节轨迹→下发」全流程），下位机（Windows）跑 `arm_real_server`（pyserial 直连 COM4）。
...
```bash
# 下位机 Windows：直接起 robot server（pyserial 直连 COM4，监听 9001；首次需 pip install pyserial）
...
上位机经 TCP 把关节轨迹下发到下位机，详细部署与排错见 README「四、实机控制」。链路（直连，已去掉 WSL/bridge/portproxy）：
`client →(tailscale 9001)→ robot_server(Windows，pyserial 直连) → COM4 → 主控板`。
...
**`arm_real_server/robot_server.py`** — 下位机 Windows 端，监听 9001，**pyserial 直连 COM4**  
`handle_request` 分发请求；`make_arm_cmd` 做关节限幅并转成底层 `T=122` 串口指令；trajectory 在独立线程**逐点闭环执行**：`send_point_and_wait` 发一个点后轮询 `T=105` 状态，检测「角度收敛」（b/s/e/w 连续几次几乎不再变=机械臂已停）才发下一个；探测不到状态返回时回退按 `dt` 定时。串口读写用 `ser_lock` 串行化。状态存 `server_state`（idle/running/done/stopped/error），`stop_event` 中断（只阻止后续点，非物理急停）。  
...
- 串口物理要稳：机械臂猛动曾拉扯 USB 线 / 供电波动导致瞬断（数据线没插好时 `T=105` 返回全 `\x00`）；`query_state` 已捕获 `SerialException`、trajectory 出错记 error 不崩进程。

⚠️ `robot_server.py` 部署在**下位机 Windows**（需 `pip install pyserial`），改完手动同步过去；旧的 WSL + serial_bridge + portproxy 架构已废弃。
